{-# LANGUAGE ForeignFunctionInterface, TemplateHaskell, DeriveDataTypeable, GeneralizedNewtypeDeriving, ScopedTypeVariables #-}
module ViperVM.Platform.OpenCL (
   ...
   getPlatformInfos',
   -- Device
   getDeviceEndianness, getDeviceEndianness',
   -- Contexts
...
-- | Get device memory endianness
getDeviceEndianness :: Library -> Device -> IO (Either CLError Endianness)
getDeviceEndianness lib dev = fmap toEndianness <$> getDeviceInfoBool lib CL_DEVICE_ENDIAN_LITTLE dev
   where
      toEndianness True = LittleEndian
      toEndianness False = BigEndian

-- | Get device memory endianness (throw an exception on error)
getDeviceEndianness' :: Library -> Device -> IO Endianness
getDeviceEndianness' lib dev = toException <$> getDeviceEndianness lib dev
