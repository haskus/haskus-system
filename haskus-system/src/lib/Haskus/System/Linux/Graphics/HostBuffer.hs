...
createHostBuffer :: MonadInIO m => Handle -> Word32 -> Word32 -> Word32 -> Word32 -> Excepts '[ErrorCode] m HostBuffer
...
destroyHostBuffer :: MonadInIO m => Handle -> HostBuffer -> Excepts '[ErrorCode] m ()
destroyHostBuffer hdl buffer = do
   let s = StructDestroyDumb (cdHandle buffer)
   void (ioctlDestroyHostBuffer s hdl)

-- | Map a host buffer
mapHostBuffer :: MonadInIO m => Handle -> HostBuffer -> Excepts '[ErrorCode] m HostBufferMap
