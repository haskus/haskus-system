...
writeStr :: MonadInIO m => Handle -> String -> Flow '[ErrorCode] m ()
writeStr fd = writeBuffer fd . stringEncodeUtf8

-- | Write a String with a newline character in the given
-- file descriptor
writeStrLn :: MonadInIO m => Handle -> String -> Flow '[ErrorCode] m ()
...
-- read
readChar :: MonadInIO m => Handle -> Flow ReadErrors' m Char
