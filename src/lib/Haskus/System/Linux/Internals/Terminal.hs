...
--
-- TCGETS
ttyGetConfig :: (MonadInIO m) => Handle -> Flow m '[TermConfig,ErrorCode]
ttyGetConfig = ioctlReadCmd (rawIoctlCommand 0x5401)

-- | Set serial port settings
--
-- TCSETS
ttySetConfig :: (MonadInIO m) => TermConfig -> Handle -> Flow m '[(),ErrorCode]
ttySetConfig = ioctlWriteCmd (rawIoctlCommand 0x5402)

-- | Allow the output buffer to drain, and set serial port settings
--
-- TCSETSW
ttySetConfigDrain :: (MonadInIO m) => TermConfig -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TCSETSF
ttySetConfigFlush :: (MonadInIO m) => TermConfig -> Handle -> Flow m '[(),ErrorCode]
...
-- 
-- TCSBRK
ttyDrain :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TCSBRKP
ttyDrainDuration :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TCSSBRK
ttyDrainStart :: (MonadInIO m) => Handle -> Flow m '[(),ErrorCode]
...
--
-- TCSCBRK
ttyDrainStop :: (MonadInIO m) => Handle -> Flow m '[(),ErrorCode]
...
--
-- TCXONC
ttyFlowControl :: (MonadInIO m) => FlowControl -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TCFLSH
ttyFlush :: (MonadInIO m) => FlushArg -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCEXCL
ttyEnableExclusive :: (MonadInIO m) => Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCGEXCL
ttyIsExclusive :: (MonadInIO m) => Handle -> Flow m '[Bool,ErrorCode]
ttyIsExclusive fd = ioctlRead 0x54 0x40 fd >.-.> (/= (0::Int))

-- | Disable exclusive mode.
--
-- TIOCNXCL
ttyDisableExclusive :: (MonadInIO m) => Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCSCTTY
ttySetControllingTerminal :: (MonadInIO m) => Bool -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCNOTTY
ttyRemoveControllingTerminal :: (MonadInIO m) => Handle -> Flow m '[(),ErrorCode]
ttyRemoveControllingTerminal = ioctlSignalCmd (rawIoctlCommand 0x5422) False

-- | Get the process group ID of the foreground process group on this terminal.
--
-- TIOCGPGRP
ttyGetForegroundProcessGroupId :: (MonadInIO m) => Handle -> Flow m '[GroupID,ErrorCode]
ttyGetForegroundProcessGroupId fd = ioctlReadCmd (rawIoctlCommand 0x540F) fd >.-.> GroupID

-- | Set the foreground process group ID of this terminal.
--
-- TIOCSPGRP
ttySetForegroundProcessGroupId :: (MonadInIO m) => GroupID -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCGSID
ttyGetSessionId :: (MonadInIO m) => Handle -> Flow m '[SessionID,ErrorCode]
...
--
-- FIONREAD/TIOCINQ
ttyGetInputSize :: (MonadInIO m) => Handle -> Flow m '[Int,ErrorCode]
ttyGetInputSize = ioctlReadCmd (rawIoctlCommand 0x541B)

-- | Get the number of bytes in the output buffer
--
-- TIOCOUTQ
ttyGetOutputSize :: (MonadInIO m) => Handle -> Flow m '[Int,ErrorCode]
ttyGetOutputSize = ioctlReadCmd (rawIoctlCommand 0x5411)

-- | Insert the given byte in the input queue
--
-- TIOCSTI
ttyFakeInput :: (MonadInIO m) => Word8 -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCGWINSZ
ttyGetWindowSize :: (MonadInIO m) => Handle -> Flow m '[WindowSize,ErrorCode]
ttyGetWindowSize = ioctlReadCmd (rawIoctlCommand 0x5413)

-- | Set window size
--
-- TIOCSWINSZ
ttySetWindowSize :: (MonadInIO m) => WindowSize -> Handle -> Flow m '[(),ErrorCode]
ttySetWindowSize = ioctlWriteCmd (rawIoctlCommand 0x5414)

-- | Get modem bits
--
-- TIOCMGET
ttyGetModemBits :: (MonadInIO m) => Handle -> Flow m '[Int,ErrorCode]
ttyGetModemBits = ioctlReadCmd (rawIoctlCommand 0x5415)

-- | Set modem bits
--
-- TIOCMSET
ttySetModemBits :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCMBIC
ttyClearSomeModemBits :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
ttyClearSomeModemBits = ioctlWriteCmd (rawIoctlCommand 0x5417)

-- | Set the indicated modem bits
--
-- TIOCMBIS
ttySetSomeModemBits :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
...
--
-- FIXME: blocking call!
ttyModemWait :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCGSOFTCAR
ttyGetSoftwareCarrierFlag :: (MonadInIO m) => Handle -> Flow m '[Int,ErrorCode]
...
--
-- TIOCSSOFTCAR
ttySetSoftwareCarrierFlag :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCCONS
ttyRedirectConsole :: (MonadInIO m) => Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCPKT
ttySetPacketMode :: (MonadInIO m) => Bool -> Handle -> Flow m '[(),ErrorCode]
ttySetPacketMode b = ioctlWriteCmd (rawIoctlCommand 0x5420) (if b then 1 else 0 :: Int)

-- | Get packet mode
--
-- TIOCGPKT
ttyGetPacketMode :: (MonadInIO m) => Handle -> Flow m '[Bool,ErrorCode]
...
--
-- TIOCGETD
ttyGetLineDiscipline :: (MonadInIO m) => Handle -> Flow m '[Int,ErrorCode]
...
--
-- TIOCSETD
ttySetLineDiscipline :: (MonadInIO m) => Int -> Handle -> Flow m '[(),ErrorCode]
ttySetLineDiscipline = ioctlWriteCmd (rawIoctlCommand 0x5423)

-- | Get extended configuration
--
-- TCGETS2
ttyGetConfigExt :: (MonadInIO m) => Handle -> Flow m '[TermConfigExt,ErrorCode]
ttyGetConfigExt = ioctlRead 0x54 0x2A

-- | Set extended configuration
--
-- TCSETS2
ttySetConfigExt :: (MonadInIO m) => TermConfigExt -> Handle -> Flow m '[(),ErrorCode]
ttySetConfigExt = ioctlWrite 0x54 0x2B

-- | Set extended configuration, allow the output buffer to drain
--
-- TCSETSW2
ttySetConfigDrainExt :: (MonadInIO m) => TermConfigExt -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TCSETSF2
ttySetConfigFlushExt :: (MonadInIO m) => TermConfigExt -> Handle -> Flow m '[(),ErrorCode]
ttySetConfigFlushExt = ioctlWrite 0x54 0x2D

-- | Get Pty number (of pty-mux device)
--
-- TIOCGPTN
ttyGetPtyNumber :: (MonadInIO m) => Handle -> Flow m '[Word32,ErrorCode]
ttyGetPtyNumber = ioctlRead 0x54 0x30

-- | Lock/unlock Pty
--
-- TIOCSPTLCK
ttySetPtyLock :: (MonadInIO m) => Bool -> Handle -> Flow m '[(),ErrorCode]
ttySetPtyLock b = ioctlWrite 0x54 0x31 (if b then 1 else 0 :: Int)

-- | Get Pty lock state
--
-- TIOCGPTLCK
ttyGetPtyLock :: (MonadInIO m) => Handle -> Flow m '[Bool,ErrorCode]
ttyGetPtyLock fd = ioctlRead 0x54 0x39 fd >.-.> (/= (0 :: Int32))

-- | Get primary device node of /dev/console
--
-- TIOCGDEV
ttyGetConsoleDevice :: (MonadInIO m) => Handle -> Flow m '[Word32,ErrorCode]
ttyGetConsoleDevice = ioctlRead 0x54 0x32

-- | pty: generate signal
--
-- TIOCSIG
ttyGeneratePtySignal :: (MonadInIO m) => Int32 -> Handle -> Flow m '[(),ErrorCode]
...
--
-- TIOCGLCKTRMIOS
ttyGetConfigLock :: (MonadInIO m) => Handle -> Flow m '[TermConfig,ErrorCode]
...
--
-- TIOCSLCKTRMIOS
ttySetConfigLock :: (MonadInIO m) => TermConfig -> Handle -> Flow m '[(),ErrorCode]
ttySetConfigLock = ioctlWriteCmd (rawIoctlCommand 0x5457)

-- | Enable/disable non-blocking mode (disabled by default)
--
-- FIONBIO
ttySetNonBlockingMode :: (MonadInIO m) => Bool -> Handle -> Flow m '[(),ErrorCode]
...
--
-- FIOASYNC
ttySetAsyncMode :: (MonadInIO m) => Bool -> Handle -> Flow m '[(),ErrorCode]
